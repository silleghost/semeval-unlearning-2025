...
def get_args_and_verify():
    parser = argparse.ArgumentParser(
        description="Script to run inference and evaluation"
    )
    parser.add_argument(
        "--data_path", help="Path to unlearning dataset containing jsonl files"
    )
    ...
    parser.add_argument(
        ...
        required=False,
        default=None,
        help="Path to store inference files and evaluation results",
    )
    parser.add_argument(
        "--mia_data_path",
        required=False,
        default=None,
        help="Path to member and nonmember jsonl files for MIA attack",
    )
    parser.add_argument(
        "--mmlu_metrics_file_path",
        required=False,
        default=None,
        help="Path to metrics.json file generated by MMLU",
    )
    parser.add_argument(
        "--max_new_tokens",
        required=False,
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Skip inference and compute metrics from inference files",
    )
    parser.add_argument(
        "--seed", required=False, default=42, help="Random seed for experiments"
    )
    parser.add_argument(
        "--keep_files",
        required=False,
    ...
    if args.mia_data_path is not None:
        ...
        print(
            "WARNING: MIA data path not provided, your final evaluation metric includes MIA test set performance so please rerun with this option to get the accurate performance. Proceeding for now"
        )
    ...
    if args.mmlu_metrics_file_path is not None:
        ...
        print(
            "WARNING: MMLU metrics file not provided, your final evaluation metric includes MMLU aggregate performance so please run this test to get the accurate performance. Proceeding for now"
        )
...
def inference(args, model, tokenizer):
    ...
    for split, train_file in [("retain", retain_file), ("forget", forget_file)]:
        ...
        if args.debug:
            print(
                [len(value) for value in output_dic[accelerator.process_index].values()]
            )
...
def compute_metrics(args):
    ...
    if "mmlu_average" in results and "mia_loss_acc" in results:
        if results["mmlu_average"] < 0.371:
            ...
            print(
                f"[WARNING] The MMLU average for the provided checkpoint is below threshold. If this happens your model may not be considered in final challenge ranking."
            )

        mia_final_score = 1 - abs(results["mia_loss_acc"] - 0.5) * 2
        results["mia_final_score"] = mia_final_score
        results["aggregate-score"] = mean(
            [task_aggregate, results["mmlu_average"], mia_final_score]
        )
